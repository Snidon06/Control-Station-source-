...
namespace Test_1
{
    public partial class Scherm_1 : Form
    {
        ...
        public Scherm_1()
        {
            ...
            ((Control)webView21).Enabled = false;
            ((Control)webView22).Enabled = false;
            ...
            this.Load += Scherm_1_Load;
        }
        //c new
private async void Scherm_1_Load(object sender, EventArgs e)
{
    try {
        ...
        webView22.NavigationCompleted += WebView22_NavigationCompleted;
                ...
                Debug.WriteLine("Before navigation: during init");

                // Initial navigation
                webView21.CoreWebView2.Navigate("https://www.openstreetmap.org/#map=2/50.8/5.6");
                Debug.WriteLine("Navigation executed.");
            }
    catch (Exception ex) {
        ...
        //new c
        // Add these event handlers after initialization
...
private void WebView21_NavigationCompleted(object sender, CoreWebView2NavigationCompletedEventArgs e)
{
    if (e.IsSuccess)
    {
        ...
        Debug.WriteLine("WebView21 navigation completed successfully");
    }
    else
    {
        Debug.WriteLine("WebView21 navigation failed: " + e.WebErrorStatus);
    }
}
...
private void WebView22_NavigationCompleted(object sender, CoreWebView2NavigationCompletedEventArgs e)
{
    if (e.IsSuccess)
    {
        ...
        Debug.WriteLine("WebView22 navigation completed successfully");
    }
    else
    {
        Debug.WriteLine("WebView22 navigation failed: " + e.WebErrorStatus);
    }
}
...
//end c
        ...
        private void OpenClose_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Debug.WriteLine("Before navigation: after waypoint claer");
        ...
        private void Timer2_Tick(object sender, EventArgs e)
        {
            ...
            if (first_receive == 1)
            {
                ...
                if (error == 0) textBox3.Text = "No error and " + location_address;
        ...
        private async void Location_update_timer_Tick(object sender, EventArgs e)
        {
            //bejar hadlo
            Debug.WriteLine("Timer Tick: Checking location update...");
            
            // Ensure there is valid GPS data
            if (first_receive == 1 && number_used_sats > 4 && l_lat_gps != 0)
            {
                Debug.WriteLine("Valid telemetry data detected. Updating location...");
                ...
                Debug.WriteLine("New location URL: " + location_address);

                try
                {
                    if (webView21.CoreWebView2 != null && webView22.CoreWebView2 != null)
                    {
                        if (webbrouwser_active == 0)
                        {
                            Debug.WriteLine("Navigating WebView21");
                            webView21.CoreWebView2.Navigate(location_address);
                            await Task.Delay(500); // 0.5s delay before switching
                            webView21.Visible = true;
                            webView22.Visible = false;
                        ...
                        {
                            Debug.WriteLine("Navigating WebView22");
                            webView22.CoreWebView2.Navigate(location_address);
                            await Task.Delay(500); // 0.5s delay before switching
                    ...
                    {
                        Debug.WriteLine("WebView2 not ready. Skipping navigation.");
                    }
                }
                catch (Exception ex)
                {
                    Debug.WriteLine("Error during WebView2 navigation: " + ex.Message);
                }
            }
            else
            {
                Debug.WriteLine("Invalid GPS data or not receiving telemetry. Skipping update.");
            }
        }
        ...
        private void Scherm_1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                if (click_lat > webView21.Location.Y && click_lat < webView21.Location.Y + webView21.Size.Height
                && click_lon > webView21.Location.X && click_lon < webView21.Location.X + webView21.Size.Width)
                {
                    ...
                    click_lat -= 35;
                    click_lat -= (webView21.Size.Height / 2) + webView21.Location.Y;
                    click_lon -= (webView21.Size.Width / 2) + webView21.Location.X;
                    if (zoom - 14 == 5)
                    {
